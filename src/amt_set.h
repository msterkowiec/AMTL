...
namespace amt
{
	...
	class set : public std::set<__AMT_TRY_WRAP_TYPE__(T, PREVENT_AMT_WRAP, Alloc), Compare, __AMT_CHANGE_ALLOCATOR_IF_NEEDED__(T, PREVENT_AMT_WRAP, Alloc)>
	{
		...
		public:
			...
			__FORCEINLINE__ IteratorBase(ITER it, const set* pSet) : ITER()
			{
				*((ITER*)this) = it;
				m_pSet = pSet;
			...
			__FORCEINLINE__ void AssertIsValid(const set* pSet = nullptr) const
			{
			...
			{			
				SAssertNotBegin<IsRevIter<Base, ITER>::value + IsConstRevIter<Base, ITER>::value>::RunCheck<ITER>(*((ITER*)this), *m_pSet);
			}
			__FORCEINLINE__ void AssertNotEnd() const
			{
				SAssertNotEnd<IsRevIter<Base, ITER>::value + IsConstRevIter<Base, ITER>::value>::RunCheck<ITER>(*((ITER*)this), *m_pSet);
			}
	...
	public:
		...
		__FORCEINLINE__ friend bool operator == (const set& m1, const set& m2)
		{
		...
		__FORCEINLINE__ iterator find(const T& key)
		{
			CRegisterReadingThread r(*this);
		...
		__FORCEINLINE__ const_iterator find(const T& k) const
		{
			CRegisterReadingThread r(*this);
		...
		__FORCEINLINE__  size_t count(const T& k) const
		{
			CRegisterReadingThread r(*this);
		...
		__FORCEINLINE__ size_t erase(const T& key)
		{
			CRegisterWritingThread r(*this);
			++m_nCountOperInvalidateIter;
		...
		__FORCEINLINE__ void swap(set& o)
		{
			CRegisterWritingThread r(*this);
