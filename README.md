...
- AMTL will not be helpful in detecting issues related with CPU instruction reordering (that may occur when writing lockless code dependent on order of instructions of threads); what's worse, such issues will be hidden/masked when using AMTL wrappers and will not reveal in runs with AMTL (using AMTL wrappers prevents reordering)
...
- an instance of the wrapper type (X) can be used in a context that requires an implicit cast from type T (in other words: the cast from X to T should not be treated as an implicit cast but both types X and T should be treated as equivalent, whenever an implicit cast from T would be applied by compiler but usage of type X would be illegal); furhter explanation: if the original code (without AMTL) already contains some implicit cast, using an AMTL wrapper class instead of the original class might introduce yet another implicit cast, which would make the code illegal (not compilable; C++ compiler attempts to apply only one implicit cast only) - that's why it would be very convenient to make wrapper type equivalent to the original one (without need to use any implicit casts)
